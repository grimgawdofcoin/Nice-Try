...
## 2026-07-25 14:45 — NiceClip: CI proof that the frozen app finds its bundled models
**Plan:** CI proves the Whisper models are IN the package but not that the running app can FIND them — the one hop still verified only by code review, and this app has already shipped one frozen-path bug. Expose the discovered bundled-model paths via /api/health, and make the CI smoke test fail if the frozen exe reports none, so a wrong bundle path can never ship silently again.
**Phases used:** P1 P4
**Outcome:** /api/health now reports `offline_models` (sizes resolvable without network), and the CI smoke test fails the build unless the frozen exe resolves all four. Made find_bundled_whisper_model public since it's now cross-module. Verified the live endpoint in both states (no models -> [], models present -> all four). P4 PASS with one good catch: my pre-verification 'tested the CI logic' claim was a Python replay, which does not model PowerShell's 0-object pipeline collapse to $null — the zero-model branch was dead (build still failed closed via the -notin check). Fixed with `-not $found -or ...` plus a comment, and cleaned up PEP8 spacing.
